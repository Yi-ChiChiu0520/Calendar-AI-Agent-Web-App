...
from mail_utils.mime.text import MIMEText
from mail_utils.mime.multipart import MIMEMultipart
...
def email_invitation(participants: list[str]) -> list[str]:
    ...
    for participant in participants:
        logger.info(f"Prompting for mail_utils of: {participant}")
        sys.stderr.flush()
        user_input = input(f"Please enter the mail_utils of the participants: {participant} \n")
...
def add_calendar_event(event_details: EventDetails, emails: list[str]) -> str:
    ...
    try:
        ...
        event = {
            ...
            "attendees": [{"mail_utils": email} for email in emails],
            "reminders": {
              "useDefault": True,
            },
        }
...
# ----------------------------- Step 5: Send mail_utils to the participants emails -----------------------------
def send_email(to_emails: list[str], subject: str, message: str):
    ...
    try:
        ...
        logger.error(f"Failed to send mail_utils: {e}")
...
def generate_confirmation(event_details: EventDetails, calendar_link: str) -> EventConfirmation:
    ...
    completion = client.beta.chat.completions.parse(
        ...
        messages = [
            {
                "role": "system",
                "content": f"Generate a natural confirmation message for the event. Sign of with your name; Susie, and add the calendar link at the end of the mail_utils message {calendar_link}",
            },
...
def process_calendar_request(user_input: str) -> Optional[EventConfirmation]:
    ...
    calendar_link = add_calendar_event(event_details, participants_emails)
    ...
    send_confirmation = input(
        f"Could you please let me know if this mail_utils looks good to send? If Yes, then I will send this mail_utils to {emails_string}. If No, then I will output the mail_utils again and you can change it to your liking. Type [Yes\\No] \n")
    if send_confirmation.lower() == "yes":
        # call send mail_utils function
        send_email(
